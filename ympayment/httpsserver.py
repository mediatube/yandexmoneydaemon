...
q_billingv = Queue(connection=redis_conn, name='billingv', default_timeout=3600)


class S(SimpleHTTPRequestHandler):
    def _set_headers(self):
        raw_url = str(self.path)
        if raw_url.find('mediatube.xyz/generate?page=') != -1:
        ...
        try:
            ...
            if page_type == 'subscribe':
                ...
                if computed_hash == security:
                    ...
                    generated_page = re.sub('%TRANSACTIONLABEL%', invoice_label, generated_page)
                else:
    ...
    def do_POST(self):
        ...
        if incoming_hash == computed_hash:
            ...
            while job_dl.result is None:
                ...
                if job_dl.is_failed:
                    raise Exception
            job_dlv = q_billingv.enqueue(billing_service.successful_payment_callback, invoice_label, operation_id,
                                       datetime)
...
def run(server_class=HTTPServer, handler_class=S, port=443):
    server_address = ('127.0.0.1', 19997)
    httpd = server_class(server_address, handler_class)
    # httpd.socket = ssl.wrap_socket(httpd.socket, keyfile='/etc/letsencrypt/live/mediatube.xyz/privkey.pem',
    #                                certfile='/etc/letsencrypt/live/mediatube.xyz/fullchain.pem', server_side=True)
